"""
hybrid_crypto_gui_dark_full.py
...
Run:
    python hybrid_crypto_gui_dark_full.py
"""
import base64
import json
import threading
...
class HybridCryptoGUI(tk.Tk):
        ...
        try:
            if not self.logfile.exists():
                with open(self.logfile, "w", encoding="utf-8") as f:
                    f.write(f"Hybrid Crypto GUI Log — {datetime.utcnow().isoformat()}Z\n")
        except Exception:
            pass
    ...
    def _setup_style(self):
        ...
        style.configure("Card.TLabelframe", background=self.PAL["panel_bg"], relief="flat")
        ...
        # small muted labels
        style.configure("Muted.TLabel", foreground=self.PAL["muted"], background=self.PAL["panel_bg"])
    ...
    def _create_widgets(self):
        ...
        # left: sender
        sender_frame = ttk.Labelframe(main_pane, text="Sender", padding=12, style="Card.TLabelframe")
        ...
        # right: receiver
        receiver_frame = ttk.Labelframe(main_pane, text="Receiver", padding=12, style="Card.TLabelframe")
    ...
    def _build_sender_panel(self, parent):
        ...
        ttk.Button(upper, text="Generate Sender Keys", command=self._threaded_gen_sender).pack(side="left", padx=4)
        ttk.Button(upper, text="Save Sender Key", command=self._save_sender_key).pack(side="left", padx=4)
        ttk.Button(upper, text="Load Sender Key", command=self._load_sender_key).pack(side="left", padx=4)

        # plaintext area label & text (tk.Label for bg control)
        tk.Label(parent, text="Plaintext message:", bg=self.PAL["panel_bg"], fg=self.PAL["muted"]).pack(anchor="w", pady=(8, 4))
        ...
        ttk.Button(row, text="Encrypt & Send →", command=self._encrypt_and_send).pack(side="left", padx=4)
        ttk.Button(row, text="Clear message", command=lambda: self.msg_entry.delete("1.0", "end")).pack(side="left", padx=4)
        ttk.Button(row, text="Export Package (JSON)", command=self._export_package).pack(side="left", padx=4)

        # ciphertext display
        tk.Label(parent, text="Last ciphertext (Base64):", bg=self.PAL["panel_bg"], fg=self.PAL["muted"]).pack(anchor="w", pady=(6, 4))
        self.cipher_display = tk.Text(parent, width=48, height=8, wrap="none",
                                      bg=self.PAL["input_bg"], fg=self.PAL["text_fg"], insertbackground=self.PAL["text_fg"],
                                      relief="flat", bd=6)
    ...
    def _build_receiver_panel(self, parent):
        ...
        self.package_display = tk.Text(parent, width=48, height=10, wrap="none",
                                       bg=self.PAL["pkg_bg"], fg=self.PAL["text_fg"], insertbackground=self.PAL["text_fg"],
                                       relief="flat", bd=6)
        ...
        self.plain_display = tk.Text(parent, width=48, height=10, wrap="word",
                                     bg=self.PAL["plain_bg"], fg=self.PAL["text_fg"], insertbackground=self.PAL["text_fg"],
                                     relief="flat", bd=6)
